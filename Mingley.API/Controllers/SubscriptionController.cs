...
public class SubscriptionsController : ControllerBase
{
    ...
    public async Task<IActionResult> Plans()
    {
        ...
        // Always prepend the Free plan so frontend always has something to show
        var freePlan = new
        {
            id = "free",
            name = "Free",
            ...
            durationDays = 0,
            isPopular = false,
            features = new[] { "3 free messages per chat", "1 Super Like/day", "Basic matching", "Standard support" },
            superLikesPerDay = 1,
            boostsPerMonth = 0,
        ...
        return Ok(ApiResponse<object>.Ok(new { plans = plans.Prepend((object)freePlan).ToList() }));
    }

    // UPDATED: Returns Free plan info when user has no active subscription
    [HttpGet("status")]
    public async Task<IActionResult> Status()
    {
        ...
        if (s == null)
        {
            return Ok(ApiResponse<object>.Ok(new
            {
                ...
                planId = "free",
                daysRemaining = 0,
                features = new[] { "3 free messages per chat", "1 Super Like/day", "Basic matching" },
            }));
        }
    ...
    public async Task<IActionResult> Subscribe([FromBody] SubscribeRequest req)
    {
        ...
        return Ok(ApiResponse<SubscribeResponse>.Ok(r, $"🌟 {r.PlanName} activated! Enjoy premium features."));
    }
    ...
    public async Task<IActionResult> Cancel(Guid subscriptionId, [FromBody] CancelRequest req)
    {
        ...
        return Ok(ApiResponse.Ok("Subscription cancelled. You have been moved to the Free plan."));
    }
}
