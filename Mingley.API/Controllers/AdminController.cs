...
public class AdminController : ControllerBase
{
    ...
    public async Task<IActionResult> Dashboard()
    {
        ...
        {
            users = new { totalUsers, activeUsers, premiumUsers, onlineUsers, newUsersToday, managedUsers, suspendedUsers },
            activity = new { totalMatches, matchesToday, totalMessages, callSessions, totalSuperChats },
            finance = new { superChatRevenue, totalCommissions, totalCoinsDeducted, totalCoinsIssued, totalInCirculation, pendingDeposits, pendingWithdrawals },
        }));
    }
    ...
    public async Task<IActionResult> GetUser(Guid id)
    {
        ...
        return Ok(ApiResponse<object>.Ok(new
        {
            user = new
            {
                ...
                Subscription = user.Subscription == null ? null : new { PlanName = user.Subscription.Plan?.Name, user.Subscription.StartDate, user.Subscription.EndDate, user.Subscription.IsActive },
            },
    ...
    {
        if (await _db.Users.IgnoreQueryFilters().AnyAsync(u => u.Email == req.Email || u.Phone == req.Phone))
        ...
        {
            _db.CoinTransactions.Add(new CoinTransaction { UserId = user.Id, Coins = req.InitialCoins, Direction = "credit", Description = "Admin created account bonus", TransactionType = "admin_grant" });
            await _db.SaveChangesAsync();
        }

        return StatusCode(201, ApiResponse<object>.Created(new { id = user.Id.ToString(), user.Email, user.FullName, user.Role }, "User created."));
    }
    ...
    public async Task<IActionResult> GrantSubscription(Guid id, [FromBody] GrantSubRequest req)
    {
        ...
        if (Guid.TryParse(req.PlanId, out var planGuid)) plan = await _db.SubscriptionPlans.FindAsync(planGuid);
        plan ??= await _db.SubscriptionPlans.FirstOrDefaultAsync(p => p.IsActive && !p.IsDeleted && p.Name.ToLower() == req.PlanId.ToLower().Trim());
        ...
        _db.UserSubscriptions.Add(new UserSubscription { UserId = id, PlanId = plan.Id, EndDate = DateTime.UtcNow.AddDays(days), IsActive = true, AutoRenew = false });
    ...
    {
        var q = _db.Messages.IgnoreQueryFilters().Include(m => m.Sender).Where(m => m.SenderId == id);
        if (chatId.HasValue) q = q.Where(m => m.ChatId == chatId.Value);

        var msgs = await q.OrderByDescending(m => m.CreatedAt).Take(100)
            .Select(m => new { id = m.Id.ToString(), m.Text, m.Type, m.ImageUrl, m.CoinAmount, m.CreatedAt, m.CoinsDeducted, m.IsDeleted, chatId = m.ChatId.ToString(), Sender = new { m.Sender!.FullName, m.Sender.Avatar } })
            .ToListAsync();
    ...
    {
        var q = _db.Users.IgnoreQueryFilters().Where(u => u.IsCreatedByAdmin);
        ...
        var users = await q.OrderByDescending(u => u.CreatedAt).Skip((page - 1) * limit).Take(limit)
            .Select(u => new { id = u.Id.ToString(), u.FullName, u.Email, u.Phone, u.Gender, u.Avatar, u.IsActive, u.IsSuspended, u.SuspendReason, u.SuspendedAt, u.IsVerified, u.IsPremium, u.CoinBalance, u.TotalEarned, u.CreatedAt, u.LastActiveAt })
            .ToListAsync();
    ...
    public async Task<IActionResult> CreateManagedUser([FromBody] CreateUserRequest req)
    {
        ...
        if (req.InitialCoins > 0)
        {
            _db.CoinTransactions.Add(new CoinTransaction { UserId = user.Id, Coins = req.InitialCoins, Direction = "credit", Description = "Admin managed account bonus", TransactionType = "admin_grant" });
            await _db.SaveChangesAsync();
        }

        return StatusCode(201, ApiResponse<object>.Created(new { id = user.Id.ToString(), user.Email, user.FullName, user.CoinBalance, isCreatedByAdmin = true }, $"Managed user {user.FullName} created."));
    }
    ...
    public async Task<IActionResult> SuspendUser(Guid id, [FromBody] SuspendRequest req)
    {
        var user = await _db.Users.IgnoreQueryFilters().FirstOrDefaultAsync(u => u.Id == id)
            ?? throw new InvalidOperationException("User not found.");
        user.IsSuspended = true; user.IsActive = false;
        user.SuspendedAt = DateTime.UtcNow; user.SuspendReason = req.Reason ?? "Suspended by admin";
        user.SuspendedBy = Me.ToString(); user.UpdatedAt = DateTime.UtcNow;
        await _db.SaveChangesAsync();
        return Ok(ApiResponse<object>.Ok(new { userId = id.ToString(), isSuspended = true, reason = user.SuspendReason, suspendedAt = user.SuspendedAt }, $"User {user.FullName} has been suspended."));
    }
    ...
    public async Task<IActionResult> PauseUser(Guid id, [FromBody] SuspendRequest req)
    {
        var user = await _db.Users.IgnoreQueryFilters().FirstOrDefaultAsync(u => u.Id == id)
            ?? throw new InvalidOperationException("User not found.");
        user.IsActive = false; user.SuspendReason = req.Reason ?? "Paused by admin"; user.UpdatedAt = DateTime.UtcNow;
        await _db.SaveChangesAsync();
        return Ok(ApiResponse<object>.Ok(new { userId = id.ToString(), isActive = false, reason = user.SuspendReason }, $"User {user.FullName} has been paused."));
    }
    ...
    public async Task<IActionResult> ResumeUser(Guid id)
    {
        var user = await _db.Users.IgnoreQueryFilters().FirstOrDefaultAsync(u => u.Id == id)
            ?? throw new InvalidOperationException("User not found.");
        user.IsSuspended = false; user.IsActive = true;
        user.SuspendedAt = null; user.SuspendReason = null; user.SuspendedBy = null; user.UpdatedAt = DateTime.UtcNow;
        await _db.SaveChangesAsync();
        return Ok(ApiResponse<object>.Ok(new { userId = id.ToString(), isSuspended = false, isActive = true }, $"User {user.FullName} has been resumed."));
    }
    ...
    public async Task<IActionResult> GetUserFullLogs(Guid id)
    {
        ...
        var messages = await _db.Messages.IgnoreQueryFilters().Where(m => m.SenderId == id).OrderByDescending(m => m.CreatedAt).Take(100)
            .Select(m => new { id = m.Id.ToString(), chatId = m.ChatId.ToString(), text = m.IsDeleted ? "[deleted]" : m.Text, m.Type, m.CoinAmount, m.CreatedAt, m.IsDeleted }).ToListAsync();

        var calls = await _db.CallSessions.Include(c => c.Caller).Include(c => c.Receiver)
            .Where(c => c.CallerId == id || c.ReceiverId == id).OrderByDescending(c => c.CreatedAt).Take(50)
            .Select(c => new { id = c.Id.ToString(), c.CallType, c.Status, duration = c.DurationSeconds, coinsUsed = c.CoinsDeducted, direction = c.CallerId == id ? "outgoing" : "incoming", otherUser = c.CallerId == id ? c.Receiver!.FullName : c.Caller!.FullName, c.AnsweredAt, c.EndedAt, c.CreatedAt }).ToListAsync();

        var transactions = await _db.CoinTransactions.Where(t => t.UserId == id).OrderByDescending(t => t.CreatedAt).Take(100)
            .Select(t => new { id = t.Id.ToString(), t.Coins, t.Direction, t.Description, t.TransactionType, t.ReferenceId, t.CreatedAt }).ToListAsync();

        var superchats = await _db.SuperChats.Include(s => s.FromUser).Include(s => s.ToUser)
            .Where(s => s.FromUserId == id || s.ToUserId == id).OrderByDescending(s => s.CreatedAt).Take(50)
            .Select(s => new { id = s.Id.ToString(), direction = s.FromUserId == id ? "sent" : "received", s.Message, s.CoinAmount, s.IsResponded, otherUser = s.FromUserId == id ? s.ToUser!.FullName : s.FromUser!.FullName, s.CreatedAt }).ToListAsync();

        var deposits = await _db.DepositRequests.Where(d => d.UserId == id).OrderByDescending(d => d.CreatedAt).Take(20).Select(d => new { id = d.Id.ToString(), d.UtrId, d.RequestedCoins, d.Status, d.CreatedAt }).ToListAsync();
        var withdrawals = await _db.WithdrawalRequests.Where(w => w.UserId == id).OrderByDescending(w => w.CreatedAt).Take(20).Select(w => new { id = w.Id.ToString(), w.Coins, w.BankOrUpi, w.Status, w.CreatedAt }).ToListAsync();
        ...
        return Ok(ApiResponse<object>.Ok(new
        {
            user = new { id = user.Id.ToString(), user.FullName, user.Email, user.Phone, user.Gender, user.Avatar, user.CoinBalance, user.TotalEarned, user.IsSuspended, user.SuspendReason, user.IsCreatedByAdmin, user.IsTrending, user.CreatedAt, user.LastActiveAt },
            stats = new { totalMessages = messages.Count, totalCalls = calls.Count, totalTransactions = transactions.Count, totalSuperChats = superchats.Count, totalCoinsSpent = totalSpent, totalCoinsEarned = totalEarned, netCoins = totalEarned - totalSpent },
            messages,
            calls,
            transactions,
            superchats,
    ...
    public async Task<IActionResult> GetDeposits([FromQuery] string status = "pending")
    {
        ...
        var list = await q.OrderByDescending(d => d.CreatedAt).Take(100)
            .Select(d => new { id = d.Id.ToString(), d.UtrId, d.ScreenshotUrl, d.RequestedCoins, d.Status, d.AdminNote, d.CreatedAt, User = new { d.User!.FullName, d.User.Email, d.User.CoinBalance } })
            .ToListAsync();
    ...
    {
        var dep = await _db.DepositRequests.Include(d => d.User).FirstOrDefaultAsync(d => d.Id == id && d.Status == "pending")
            ?? throw new InvalidOperationException("Deposit not found or already processed.");
        dep.Status = "approved"; dep.AdminNote = req.Note;
        dep.User!.CoinBalance += dep.RequestedCoins ?? 0; dep.User.UpdatedAt = DateTime.UtcNow;
        _db.CoinTransactions.Add(new CoinTransaction { UserId = dep.UserId, Coins = dep.RequestedCoins ?? 0, Direction = "credit", Description = $"Deposit approved (UTR: {dep.UtrId})", TransactionType = "deposit" });
        await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> RejectDeposit(Guid id, [FromBody] AdminNoteRequest req)
    {
        ...
        dep.Status = "rejected"; dep.AdminNote = req.Note;
        await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> GetWithdrawals([FromQuery] string status = "pending")
    {
        ...
        var list = await q.OrderByDescending(w => w.CreatedAt).Take(100)
            .Select(w => new { id = w.Id.ToString(), w.Coins, w.BankOrUpi, w.Status, w.AdminNote, w.CreatedAt, User = new { w.User!.FullName, w.User.Email, w.User.Gender, w.User.TotalEarned } })
            .ToListAsync();
    ...
    public async Task<IActionResult> ApproveWithdrawal(Guid id, [FromBody] AdminNoteRequest req)
    {
        ...
        wr.Status = "approved"; wr.AdminNote = req.Note;
        await _db.SaveChangesAsync();
    ...
    {
        var wr = await _db.WithdrawalRequests.Include(w => w.User).FirstOrDefaultAsync(w => w.Id == id && w.Status == "pending")
            ?? throw new InvalidOperationException("Withdrawal not found.");
        wr.Status = "rejected"; wr.AdminNote = req.Note;
        wr.User!.CoinBalance += wr.Coins; wr.User.UpdatedAt = DateTime.UtcNow;
        _db.CoinTransactions.Add(new CoinTransaction { UserId = wr.UserId, Coins = wr.Coins, Direction = "credit", Description = "Withdrawal rejected — coins refunded", TransactionType = "refund" });
        await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> GetReports([FromQuery] string status = "pending")
    {
        ...
        var list = await q.OrderByDescending(r => r.CreatedAt).Take(100)
            .Select(r => new { id = r.Id.ToString(), r.Reason, r.Description, r.Status, r.CreatedAt, Reporter = new { r.Reporter!.FullName, r.Reporter.Email }, ReportedUser = new { r.ReportedUser!.FullName, r.ReportedUser.Email } })
            .ToListAsync();
    ...
    {
        var report = await _db.Reports.FindAsync(id) ?? throw new InvalidOperationException("Report not found.");
        report.Status = "resolved"; report.UpdatedAt = DateTime.UtcNow;
        await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> GetSuperChats([FromQuery] int page = 1, [FromQuery] bool? isResponded = null)
    {
        ...
        var list = await q.OrderByDescending(s => s.CreatedAt).Skip((page - 1) * 30).Take(30)
            .Select(s => new { id = s.Id.ToString(), s.Message, s.CoinAmount, s.CompanyRevenue, s.GirlCommission, s.IsResponded, s.RespondedAt, s.CreatedAt, FromUser = new { s.FromUser!.FullName, s.FromUser.Email }, ToUser = new { s.ToUser!.FullName, s.ToUser.Email } })
            .ToListAsync();
    ...
    public async Task<IActionResult> Broadcast([FromBody] BroadcastRequest req)
    {
        ...
        foreach (var uid in userIds)
            await _notifs.CreateAsync(uid, req.Title, req.Body, "broadcast");
    ...
    {
        await _notifs.CreateAsync(id, req.Title, req.Body, "admin");
    ...
    public async Task<IActionResult> CoinStats()
    {
        ...
        return Ok(ApiResponse<object>.Ok(new { totalInCirculation, totalIssued, totalBurned, pendingDeps, superChatRevenue, superChatCommission, topSpenders }));
    }
    ...
    {
        var q = _db.CallSessions.Include(c => c.Caller).Include(c => c.Receiver).AsQueryable();
        if (!string.IsNullOrWhiteSpace(callType)) q = q.Where(c => c.CallType == callType);

        var total = await q.CountAsync();
        var calls = await q.OrderByDescending(c => c.CreatedAt).Skip((page - 1) * 30).Take(30)
            .Select(c => new { id = c.Id.ToString(), c.CallType, c.Status, c.DurationSeconds, c.CoinsDeducted, c.AnsweredAt, c.EndedAt, c.CreatedAt, Caller = new { c.Caller!.FullName, c.Caller.Email }, Receiver = new { c.Receiver!.FullName, c.Receiver.Email } })
            .ToListAsync();
    ...
    {
        var plans = await _db.SubscriptionPlans.IgnoreQueryFilters().OrderBy(p => p.Price)
            .Select(p => new { id = p.Id.ToString(), p.Name, p.Price, p.DurationDays, p.IsActive, p.IsPopular, p.SuperLikesPerDay, p.BoostsPerMonth, p.UnlimitedLikes, p.CanSeeWhoLiked, p.VideoCallEnabled, p.CreatedAt })
            .ToListAsync();
    ...
    // TRENDING
    // ════════════════════════════════════════════════════════════════
    [HttpPost("users/{id}/trending")]
    public async Task<IActionResult> SetTrending(Guid id, [FromBody] SetTrendingRequest req)
    {
        var user = await _db.Users.IgnoreQueryFilters().FirstOrDefaultAsync(u => u.Id == id);
        if (user == null) return NotFound(ApiResponse<object>.Fail("User not found.", 404));
        user.IsTrending = req.IsTrending;
        user.UpdatedAt = DateTime.UtcNow;
        await _db.SaveChangesAsync();
        return Ok(ApiResponse.Ok(req.IsTrending ? "User marked as trending." : "User removed from trending."));
    }
}
...
public class SetTrendingRequest { public bool IsTrending { get; set; } }
...
//        return Ok(ApiResponse<object>.Ok(new
//        {
//            user = new { id = user.Id.ToString(), user.FullName, user.Email, user.Phone, user.Gender, user.Avatar, user.CoinBalance, user.TotalEarned, user.IsSuspended, user.SuspendReason, user.IsCreatedByAdmin, user.CreatedAt, user.LastActiveAt },
//            stats = new { totalMessages = messages.Count, totalCalls = calls.Count, totalTransactions = transactions.Count, totalSuperChats = superchats.Count, totalCoinsSpent = totalSpent, totalCoinsEarned = totalEarned, netCoins = totalEarned - totalSpent },
//            messages,
//            calls,
//            transactions,
...
//    // ════════════════════════════════════════════════════════════════
//    // NOTIFICATIONS — now routes through NotificationService
//    // so FCM push + SignalR both fire, not just DB insert
//    // ════════════════════════════════════════════════════════════════
//    [HttpPost("notifications/broadcast")]
//    public async Task<IActionResult> Broadcast([FromBody] BroadcastRequest req)
//    {
...
//        // Fire-and-forget per user — routes through FCM + SignalR + DB
//        foreach (var uid in userIds)
//            await _notifs.CreateAsync(uid, req.Title, req.Body, "broadcast");
...
//    public async Task<IActionResult> NotifyUser(Guid id, [FromBody] BroadcastRequest req)
//    {
//        // Routes through FCM + SignalR + DB (was DB-only before)
//        await _notifs.CreateAsync(id, req.Title, req.Body, "admin");
//        return Ok(ApiResponse.Ok("Notification sent."));
//    }
...
//        return Ok(ApiResponse<object>.Ok(new { plans }));
//    }
//    // TRENDING — POST /v1/admin/users/{id}/trending  body: { "isTrending": true/false }
//    [HttpPost("users/{id}/trending")]
//    public async Task<IActionResult> SetTrending(Guid id, [FromBody] SetTrendingRequest req)
//    {
//        var user = await _db.Users.IgnoreQueryFilters().FirstOrDefaultAsync(u => u.Id == id);
