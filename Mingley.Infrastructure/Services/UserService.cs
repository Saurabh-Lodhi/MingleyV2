...
public class UserService : IUserService
{
    ...
    public async Task UpdateInterestsAsync(Guid userId, List<string> interests)
    {
        ...
        foreach (var name in interests.Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (interest == null) { interest = new Interest { Name = name }; _db.Interests.Add(interest); await _db.SaveChangesAsync(); }
    ...
    public static UserProfileDto MapProfile(User u) => new()
    {
        ...
        { Id = i.Id.ToString(), Url = i.Url, SortOrder = i.SortOrder, IsPrimary = i.IsPrimary }).ToList(),
        Location = u.Location == null ? null : new LocationDto { Lat = u.Location.Lat, Lng = u.Location.Lng, City = u.Location.City, Country = u.Location.Country },
        Preference = u.Preference == null ? null : new PreferenceDto
        { InterestedIn = u.Preference.InterestedIn, MinAge = u.Preference.MinAge, MaxAge = u.Preference.MaxAge, MaxDistance = u.Preference.MaxDistance, RelationshipType = u.Preference.RelationshipType, NearbyOnly = u.Preference.NearbyOnly, OnlineOnly = u.Preference.OnlineOnly, VerifiedOnly = u.Preference.VerifiedOnly, Location = u.Preference.Location },
