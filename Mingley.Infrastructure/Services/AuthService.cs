...
public class AuthService : IAuthService
{
    ...
    public async Task<RegisterResponse> RegisterAsync(RegisterRequest req)
    {
        ...
        var user = new User
        {
            ...
            PasswordHash = BCrypt.Net.BCrypt.HashPassword(req.Password),
            FullName = req.FullName,
            Gender = (req.Gender ?? "male").ToLower().Trim(),
            DateOfBirth = req.DateOfBirth?.ToUniversalTime(),
            Avatar = req.Avatar,
            OtpCode = otp,
            OtpExpiry = DateTime.UtcNow.AddMinutes(10),
    ...
    public async Task<AuthResponse> VerifyOtpAsync(VerifyOtpRequest req)
    {
        ...
        // Mark profile complete if basic info is present
        if (!string.IsNullOrWhiteSpace(user.Gender) && user.DateOfBirth.HasValue)
    ...
    private bool IsDev()
    {
        var appEnv = _cfg["App:Environment"] ?? "";
        var aspEnv = _cfg["ASPNETCORE_ENVIRONMENT"] ?? "";
        return appEnv != "Production" && aspEnv != "Production";
    }
...
//            Gender = req.Gender,
//            DateOfBirth = req.DateOfBirth?.ToUniversalTime(),
//            Avatar = req.Avatar,      // NEW: save profile picture on register
//            OtpCode = otp,
//            OtpExpiry = DateTime.UtcNow.AddMinutes(10),
//            OtpPurpose = "registration",
//            CoinBalance = MingleyDbContext.WelcomeBonus,
