...
public class WalletService : IWalletService
{
    ...
    public async Task<WalletBalanceDto> GetBalanceAsync(Guid userId)
    {
        ...
        var spent = await _db.CoinTransactions.AsNoTracking()
    ...
    public Task<List<CoinPackageDto>> GetPackagesAsync() => Task.FromResult(new List<CoinPackageDto>
    {
        ...
        new() { Id = "pkg_5000", Coins = 5000, Price = 1999, Label = "Elite",    Badge = "Best Value!",  IsPopular = false },
    });
    ...
    public async Task SubmitDepositAsync(Guid userId, DepositRequestDto req)
    {
        ...
        // FIX: Accept male/man/m all as male (case-insensitive)
        var gender = user.Gender?.ToLower().Trim() ?? "";
        var isMale = gender == "male" || gender == "man" || gender == "m";
        if (!isMale)
    ...
    public async Task SubmitWithdrawalAsync(Guid userId, WithdrawalRequestDto req)
    {
        ...
        // FIX: Accept female/woman/f/girl all as female (case-insensitive)
        var gender2 = user.Gender?.ToLower().Trim() ?? "";
        var isFemale = gender2 == "female" || gender2 == "woman" || gender2 == "f" || gender2 == "girl";
        if (!isFemale)
