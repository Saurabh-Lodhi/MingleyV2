...
public class DiscoverService : IDiscoverService
{
        ...
        Guid userId, int page, int limit, DiscoverFilters? filters = null)
    {
        ...
        // Recycle profiles the user passed on (disliked) once a cooldown has elapsed,
        // so the feed never permanently empties out once someone has swiped through
        // everyone nearby. Likes/superlikes stay excluded — those are still pending
        // a potential match, so we don't want to show them again.
        var recycleCutoff = DateTime.UtcNow.AddDays(-7);
        var swipedIds = await _db.Swipes
            .Where(s => s.SwiperId == userId && (s.Action != "dislike" || s.CreatedAt > recycleCutoff))
            .Select(s => s.TargetId)
            .ToListAsync();
        ...
        // If the radius filter wiped out everyone even though eligible people exist
        // (e.g. location just changed, or few users are actually within range), don't
        // return a blank feed — fall back to the full candidate list sorted by
        // distance so the person still sees profiles instead of a dead end.
        if (dtos.Count == 0 && scored.Count > 0)
            dtos = scored;
