...
public class DiscoverService : IDiscoverService
{
            ...
            if (mutual)
            {
                ...
                var matchDto = new MatchDto
                {
                    ...
                    User = new MatchedUserDto
                    {
                        ...
                        // Online status visible to premium users only
                        IsOnline = me.IsPremium ? (target?.IsOnline ?? false) : false,
                    },
                };
    ...
    private static DiscoverUserDto MapDiscover(User u, User me, HashSet<Guid> myInterests)
    {
        ...
        return new DiscoverUserDto
        {
            ...
            IsVerified = u.IsVerified,
            IsPremium = u.IsPremium,
            // Online status visible to premium users only (fix: issue #1)
            IsOnline = me.IsPremium ? u.IsOnline : false,
            City = u.Location?.City,
            Avatar = u.Images.FirstOrDefault(i => i.IsPrimary)?.Url
                       ?? u.Images.FirstOrDefault()?.Url ?? u.Avatar,
            Images = u.Images.OrderBy(i => i.SortOrder).Select(i => i.Url).ToList(),
...
//    // TASK 6: Default radius for free users
//    private const int DefaultRadiusKm = 100;
//    private const int MaxFreeRadiusKm = 100;
...
//        // TASK 6: VIP restrictions on filters
//        if (filters != null)
//        {
//            // Non-VIP users cannot set radius > 100km
//            if (!me.IsPremium && filters.MaxDistance.HasValue && filters.MaxDistance > MaxFreeRadiusKm)
//                throw new InvalidOperationException(
//                    $"Searching beyond {MaxFreeRadiusKm}km requires a VIP subscription. Upgrade to Gold or Platinum.");

//            // Non-VIP users cannot search by city
//            if (!me.IsPremium && !string.IsNullOrWhiteSpace(filters.City))
//                throw new InvalidOperationException("Searching by city requires a VIP subscription.");

//            // Non-VIP users cannot search globally
//            if (!me.IsPremium && filters.Global == true)
//                throw new InvalidOperationException("Global search requires a VIP subscription.");
//        }
...
//        // TASK 6: Apply distance filter in memory using Haversine formula
//        var radiusKm = filters?.MaxDistance ?? (me.IsPremium ? pref?.MaxDistance ?? DefaultRadiusKm : DefaultRadiusKm);
//        if (!me.IsPremium) radiusKm = Math.Min(radiusKm, MaxFreeRadiusKm);

//        // Global search - VIP only, no distance filter
//        if (filters?.Global == true && me.IsPremium) radiusKm = int.MaxValue;
...
//            {
//                var dto = MapDiscover(u, me, myInterestIds);
//                if (myLat.HasValue && myLng.HasValue && u.Location?.Lat.HasValue == true && u.Location?.Lng.HasValue == true)
//                {
//                    dto.Distance = HaversineKm(myLat.Value, myLng.Value,
//                                               u.Location.Lat!.Value, u.Location.Lng!.Value);
//                }
//                return (dto, u.Location?.Lat, u.Location?.Lng);
//            })
//            .Where(x => !myLat.HasValue || x.dto.Distance == null || x.dto.Distance <= radiusKm)
//            .Select(x => x.dto)
//            .OrderBy(u => u.Distance ?? double.MaxValue)
//            .ThenByDescending(u => u.IsOnline)
//            .ToList();
...
//        var images = await _db.Set<UserImage>().Where(i => otherIds.Contains(i.UserId) && !i.IsDeleted).ToListAsync();
//        var interests = await _db.Set<UserInterest>().Include(i => i.Interest).Where(i => otherIds.Contains(i.UserId)).ToListAsync();
//        var locations = await _db.Set<UserLocation>().Where(l => otherIds.Contains(l.UserId)).ToListAsync();
...
//            var msgs = m.Chat?.Messages?.Where(msg => !msg.IsDeleted).OrderByDescending(msg => msg.CreatedAt).ToList() ?? new();
//            var lastMsg = msgs.FirstOrDefault();
//            var unread = msgs.Count(msg => msg.SenderId != userId && msg.ReadAt == null);

//            var userImgs = imgByUser.TryGetValue(other.Id, out var imgs) ? imgs : new();
//            var userInts = intByUser.TryGetValue(other.Id, out var ints) ? ints : new();
//            locByUser.TryGetValue(other.Id, out var location);
...
//                MatchedAt = m.CreatedAt,
//                UnreadCount = unread,
//                User = new MatchedUserDto { Id = other.Id.ToString(), FullName = other.FullName, Avatar = primaryImg, IsOnline = other.IsOnline, LastActiveAt = other.LastActiveAt },
//                MatchedUser = new MatchedUserDto { Id = other.Id.ToString(), FullName = other.FullName, Avatar = primaryImg, IsOnline = other.IsOnline, LastActiveAt = other.LastActiveAt },
//                LastMessage = lastMsg == null ? null : new LastMessageDto
//                {
//                    Text = lastMsg.Type == "coins" ? $"💰 Sent {lastMsg.CoinAmount} coins" : lastMsg.ImageUrl != null ? "📷 Photo" : lastMsg.Text,
//                    Type = lastMsg.Type,
//                    SentAt = lastMsg.CreatedAt,
//                },
//                FullProfile = new MatchFullProfileDto
...
//                    City = location?.City,
//                    Images = userImgs.OrderBy(i => i.SortOrder).Select(i => i.Url).ToList(),
//                    Interests = userInts.Select(i => i.Interest?.Name ?? "").Where(n => !string.IsNullOrEmpty(n)).ToList(),
//                },
//            };
//        }).Where(x => x != null).Select(x => x!).ToList();
//    }
...
//        if (me == null) return new();
//        var likerIds = await _db.Swipes.Where(s => s.TargetId == userId && s.Action != "dislike").Select(s => s.SwiperId).ToListAsync();
//        var swipedByMe = await _db.Swipes.Where(s => s.SwiperId == userId).Select(s => s.TargetId).ToListAsync();
//        var pending = likerIds.Except(swipedByMe).ToList();
//        var users = await _db.Users
//            .Include(u => u.Images.Where(i => !i.IsDeleted))
//            .Include(u => u.Interests).ThenInclude(i => i.Interest)
//            .Where(u => pending.Contains(u.Id) && !u.IsDeleted).ToListAsync();
//        var myInterestIds = me.Interests.Select(i => i.InterestId).ToHashSet();
//        return users.Select(u => MapDiscover(u, me, myInterestIds)).ToList();
//    }

//    // TASK 7: Trending algorithm
//    public async Task<List<TrendingSection>> GetTrendingAsync(Guid userId)
//    {
//        var me = await _db.Users.FindAsync(userId);
//        if (me == null) return new();

//        // Get female users with stats
//        //var femaleUsers = await _db.Users
//        //    .Include(u => u.Images.Where(i => !i.IsDeleted))
//        //    .Include(u => u.Interests).ThenInclude(i => i.Interest)
//        //    .Include(u => u.Location)
...
//        // SuperChat counts per user (received)
//        var scCounts = await _db.SuperChats
//            .Where(s => userIds.Contains(s.ToUserId) && s.IsResponded)
//            .GroupBy(s => s.ToUserId)
//            .Select(g => new { UserId = g.Key, Count = g.Count(), Earned = g.Sum(s => s.GirlCommission) })
...
//                Section  = "trending",
//                Title    = "🔥 Trending Now",
//                Subtitle = "Most active profiles this week",
//                Users    = trendingUsers.OrderByDescending(u => u.TrendingScore).Take(10).ToList(),
//            },
//            new()
//            {
//                Section  = "most_popular",
//                Title    = "⭐ Most Popular",
//                Subtitle = "Girls with the most matches",
//                Users    = trendingUsers.OrderByDescending(u => u.TotalMatches).Take(10).ToList(),
//            },
//            new()
//            {
//                Section  = "top_earners",
//                Title    = "💰 Top SuperChat",
//                Subtitle = "Girls earning the most from SuperChats",
//                Users    = trendingUsers.OrderByDescending(u => u.TotalSuperChats).Take(10).ToList(),
//            },
//            new()
//            {
//                Section  = "online_now",
//                Title    = "🟢 Online Now",
//                Subtitle = "Available to chat right now",
//                Users    = trendingUsers.Where(u => u.IsOnline).OrderByDescending(u => u.TrendingScore).Take(10).ToList(),
//            },
//            new()
//            {
//                Section  = "recommended",
...
//                Users    = trendingUsers.OrderBy(_ => Guid.NewGuid()).Take(10).ToList(), // random for now
//            },
//        };
//    }
...
////    // CHANGED: now loads Interests + Images to populate FullProfile
////    public async Task<List<MatchListItemDto>> GetMatchesAsync(Guid userId, int page, int limit)
////    {
////        var matches = await _db.Matches
////            .Include(m => m.User1)
...
////            .Include(m => m.User1)
////                .ThenInclude(u => u!.Interests).ThenInclude(i => i.Interest)
////            .Include(m => m.User1)
...
////            var other = m.User1Id == userId ? m.User2! : m.User1!;
////            var msgs = m.Chat?.Messages?.Where(msg => !msg.IsDeleted)
////                           .OrderByDescending(msg => msg.CreatedAt).ToList() ?? new();
////            var lastMsg = msgs.FirstOrDefault();
////            var unread = msgs.Count(msg => msg.SenderId != userId && msg.ReadAt == null);
...
////                LastMessage = lastMsg == null ? null : new LastMessageDto
////                {
////                    Text = lastMsg.Text ?? (lastMsg.Type == "coins" ? $"💰 Sent {lastMsg.CoinAmount} coins" : lastMsg.ImageUrl != null ? "📷 Photo" : null),
////                    Type = lastMsg.Type,
////                    SentAt = lastMsg.CreatedAt,
////                },
////                // Full profile — same richness as Discover screen
////                FullProfile = new MatchFullProfileDto
////                {
////                    Id = other.Id.ToString(),
////                    FullName = other.FullName,
...
////                    Avatar = other.Images.FirstOrDefault(i => i.IsPrimary)?.Url
////                                ?? other.Images.FirstOrDefault()?.Url
////                                ?? other.Avatar,
////                    Images = other.Images.OrderBy(i => i.SortOrder).Select(i => i.Url).ToList(),
