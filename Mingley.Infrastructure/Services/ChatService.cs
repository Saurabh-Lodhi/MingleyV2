...
public class ChatService : IChatService
{
    ...
    {
        // OPTIMIZED: previously this loaded EVERY message of EVERY chat into memory
        // via .Include(c => c.Messages). Now we project only the last message,
        // its sender, and an unread count straight from the database.
        var rows = await _db.Chats
            ...
            .Select(c => new
            {
                ...
                LastMessage = c.Messages
                    ...
                    .FirstOrDefault(),
                // Selected separately so EF tracks the Sender entity; relationship
                // fixup attaches it to LastMessage.Sender (no Include-in-Select).
                ...
                UnreadCount = c.Messages
                    .Count(m => !m.IsDeleted && m.SenderId != userId && m.ReadAt == null),
                LastActivity = c.Messages
                    .Where(m => !m.IsDeleted)
                    .Max(m => (DateTime?)m.CreatedAt) ?? c.CreatedAt,
            })
        ...
        return rows.Select(x =>
        {
            ...
            return new ChatListItemDto
            {
                ...
                LastMessage = x.LastMessage == null ? null : MapMsg(x.LastMessage),
            };
        }).ToList();
    }
