...
public class CallService : ICallService
{
    ...
    private const int RingTimeoutSeconds = 30;
    ...
    public async Task<object> InitiateCallAsync(Guid callerId, string targetId, string callType)
    {
        ...
        if (caller.CoinBalance < ratePerMin)
            throw new InvalidOperationException($"Insufficient coins. Need at least {ratePerMin} coins to start a {callType} call.");

        // Cancel stale ringing calls from this caller
        var stale = await _db.CallSessions
            .Where(c => c.CallerId == callerId && c.Status == "ringing")
            .ToListAsync();
        foreach (var old in stale) { old.Status = "cancelled"; old.EndedAt = DateTime.UtcNow; old.EndReason = "new_call"; }
        ...
        // Use Random.Shared — thread-safe, no duplicate seeds
        var channelName = $"call_{session.Id}";
        uint callerUid = (uint)Random.Shared.Next(100000, 999999);
    ...
    public async Task<object> AnswerCallAsync(Guid receiverId, Guid callId)
    {
        ...
        if (session.Status is "timeout" or "declined" or "cancelled")
        ...
        uint receiverUid = (uint)Random.Shared.Next(1000000, 9999999);
    ...
    {
        // DB transaction — coin deduction + session update are atomic
        ...
        try
        {
            ...
            if (session.Status == "ended")
                return new { callId = callId.ToString(), status = "ended" };
